...
class PennyPetProcessor:
    ...
    def calculer_remboursement_pennypet(self, montant, code_acte, formule):
    ...
    def extract_lignes_from_image(self, image_bytes, formule, llm_provider="qwen"):
        ...
        # Extraction robuste du JSON, même si le LLM retourne du texte autour
        try:
            data = json.loads(content)
        except json.JSONDecodeError:
            # Utilisation d'un raw string pour éviter les warnings
            match = re.search(r"(\{.*\})", content, re.DOTALL)
            if not match:
                raise ValueError(f"Réponse LLM Vision invalide : {content!r}")
        ...
        # CORRECTION CRITIQUE: Retourner aussi content pour l'audit
        return data, content

    def process_facture_pennypet(self, file_bytes, formule_client, llm_provider="qwen"):
        # CORRECTION CRITIQUE: Récupérer à la fois data et content
        extraction, raw_content = self.extract_lignes_from_image(file_bytes, formule_client, llm_provider)
        
        # Utilise la formule détectée dans la facture si présente
        ...
        for ligne in lignes:
            ...
            remboursement = self.calculer_remboursement_pennypet(montant, code, formule)
        ...
        return {
            ...
            "llm_raw": raw_content  # CORRECTION: Utiliser raw_content
        }
